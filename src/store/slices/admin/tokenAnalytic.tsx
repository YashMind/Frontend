...
type FilterType = "daily" | "monthly" | "yearly" | "quarterly" | "bi-annual";
type Params = {
  filter?: FilterType;
};
...
>("tokenCredit/fetchUserCreditsAndTokens", async (_, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      err.response?.data?.detail || "Failed to fetch user credits"
    );
  }
});
...
export const fetchAdminTokenCreditReport = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "tokenCredit/fetchAdminTokenCreditReport",
  async ({ page = 1, perPage = 100 }, { rejectWithValue }) => {
    try {
      const response = await http.get(
        `/admin/token-credit-report?page=${page}&per_page=${perPage}`
      );

      return response.data;
    } catch (err: any) {
      return rejectWithValue(
        err.response?.data?.detail || "Failed to fetch admin token report"
      );
    }
  }
);
...
export const fetchAdminTransactions = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "tokenCredit/fetchAdminTransactions",
  async (
    { page = 1, perPage = 100, groupBy = "monthly" },
    { rejectWithValue }
  ) => {
    try {
...
export const fetchUsersByPlan = createAsyncThunk<
  ...
  Params,
  { rejectValue: string }
>("users/fetchUsersByPlan", async ({ filter = "" }, { rejectWithValue }) => {
  try {
...
const tokenCreditSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchUserCreditsAndTokens.fulfilled,
        (state, action: PayloadAction<UserCreditsAndTokenUsageResponse>) => {
      ...
      .addCase(
        fetchAdminTokenCreditReport.fulfilled,
        (state, action: PayloadAction<AdminTokenCreditReport>) => {
      ...
      .addCase(
        ...
        (state, action: PayloadAction<AdminTransactionsType>) => {
