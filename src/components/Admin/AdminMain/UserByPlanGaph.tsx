      ...
      <div className="bg-[#1A202C] p-3 shadow-lg rounded-md border border-[#2D3748] text-white">
        ...
        {payload.map((entry: any, index: number) => (
          <p key={index} className="text-sm mt-1">
            {entry.name}:{" "}
            <span className="font-semibold text-[#4ade80]">{entry.value}</span>
          </p>
        ))}
      </div>
    );
...
// Define the type for filter
type FilterType = "daily" | "monthly" | "yearly";

export default function UserByPlanGraph() {
  const dispatch = useDispatch<AppDispatch>();
  const [filter, setFilter] = useState<FilterType>("monthly"); // Add type annotation

  const usersByPlanState = useSelector(
    (state: RootState) => state.tokens?.usersByPlan
  );
  const {
    data: usersByPlan = [],
    loading = false,
  ...
  const totalUsers = usersByPlan.reduce(
    (sum, item) => sum + (item.user_count || 0),
    0
  );
  ...
  const mostPopularPlan = useMemo(() => {
    return usersByPlan.length
      ? usersByPlan.reduce(
          (max, plan) => (plan.user_count > max.user_count ? plan : max),
          usersByPlan[0]
        )
      : null;
  }, [usersByPlan]);
  ...
  const uniquePlans = useMemo(
    () => [...new Set(usersByPlan.map((item) => item.plan))],
    [usersByPlan]
  );
  const uniquePeriods = useMemo(
    () => [...new Set(usersByPlan.map((item: any) => item.period))],
    [usersByPlan]
  );
  ...
  const transformedData = useMemo(() => {
    return uniquePeriods.map((period) => {
      ...
      uniquePlans.forEach((plan: any) => {
        const matched = usersByPlan.find(
          (d: any) => d.period === period && d.plan === plan
        );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-[#252A41] p-4 rounded-lg border border-[#343B4F]">
          ...
          <p className="text-sm font-bold text-white truncate">
            {mostPopularPlan
              ? `${mostPopularPlan.plan} (${mostPopularPlan.user_count})`
              : "N/A"}
          </p>
        </div>
      </div>
            ...
            <LineChart data={transformedData}>
              ...
              {uniquePlans.map((plan, index) => (
                <Line
                  ...
                  name={plan}
                  stroke={
                    ["#4ade80", "#60a5fa", "#f87171", "#fbbf24"][index % 4]
                  }
                  strokeWidth={2}
                  dot={false}
                />
