...
const ConfigureMailDialog = ({ botId, email: defaultEmail, open, onClose }: ConfigureMailDialogProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            await dispatch(configureLeadMail({ bot_id: botId, email: email })).unwrap();
            setEmail('');
            onClose();
        } catch (err) {
        ...
        <div className="fixed inset-0 bg-black/40 bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl overflow-hidden w-full max-w-xl">
                ...
                <div className="bg-[#340555] px-4 py-2 flex justify-between items-center">
                    <h2 className="text-lg font-bold text-white">Configure Email Notifications</h2>
                    <button
                        onClick={onClose}
