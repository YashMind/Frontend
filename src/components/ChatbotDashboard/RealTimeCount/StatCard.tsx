...
const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`w-full rounded-xl p-2 px-4 text-white shadow-lg cursor-pointer ${
          isMobile && state.show ? "hidden" : ""
        }`}
        style={{
      ...
      <div
        className={
          state.show
            ? `bg-gradient-to-br from-[#1a1440] to-[#2a0e61] shadow-white w-full rounded-xl p-2 px-4 transition duration-200 ease-in-out text-white ${
                isMobile ? "relative " : "absolute top-0 left-0 z-10"
              }`
            : `hidden`
        }
        style={{
      ...
      >
        <button
          onClick={handleToggleCard}
          className="flex items-center mb-3 justify-between w-full"
        >
          <h2 className="text-center font-semibold text-base">{title}</h2>
          <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-xl">
            {icon}
