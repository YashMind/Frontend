...
export default function ApiTestPage() {
  ...
  const testAcceptInvite = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toasterSuccess("API call successful", 3000, "api-test");
      } else {
        setError(data.detail || "API call failed");
        toasterError(data.detail || "API call failed", 3000, "api-test");
      }
    } catch (err: any) {
      console.error("API call error:", err);
      setError(err.message || "An error occurred");
      toasterError(err.message || "An error occurred", 3000, "api-test");
    } finally {
      setLoading(false);
    }
  };
