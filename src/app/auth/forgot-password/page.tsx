...
const page = () => {
  ...
  const [forgotPasswordEmail, setForgotPasswordEmail] = useState<any>("");
  ...
  const handleForgotPassword = async () => {
    try {
      ...
      setForgotPasswordStatus({
        success: true,
        message:
          "Password reset link sent to your email! it will expire in 15 minutes",
      });
    } catch (error: any) {
      setForgotPasswordStatus({
        success: false,
              ...
              {forgotPasswordStatus && (
                <div
                  className={`mb-4 p-3 rounded-lg ${
                    forgotPasswordStatus.success ? "bg-green-500" : "bg-red-500"
                  }`}
                >
